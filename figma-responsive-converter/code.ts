  ...
  try {
    if (node.type === 'TEXT') {
      ...
      if (textNode.textStyleId && typeof textNode.textStyleId === 'string') {
        const currentStyle = await figma.getStyleByIdAsync(textNode.textStyleId) as TextStyle;
          ...
          for (const [fromStyle, toStyle] of Object.entries(styleMap)) {
            if (styleName.includes(fromStyle)) {
              ...
              if (newStyle) {
                ...
                await figma.loadFontAsync(styleNode.fontName as FontName);
                styleNode.remove();
      ...
      // 텍스트 스타일이 없는 경우 폰트 사이즈 기반 변환 (fallback)
        ...
        const fontWeight = (textNode.fontName as FontName).style;
